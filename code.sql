...
-- p_batas_harian DIABAIKAN di dalam function (jebolan lama) — limit
-- selalu 1, jadi client ga bisa tembak lewat RPC. Hari = WIB.
...
-- p_batas_harian DIABAIKAN di dalam function — limit selalu 1.
-- Hari dihitung Asia/Jakarta (WIB), bukan UTC.
...
-- ============================================================
-- POSTER (feed foto + caption, arsip peringatan/peringatan hari)
-- ============================================================
...
CREATE POLICY "osis_foto_poster_select" ON storage.objects FOR SELECT TO anon USING (bucket_id='osis-foto' AND (storage.foldername(name))[1]='poster');
DROP POLICY IF EXISTS "osis_foto_poster_insert" ON storage.objects;
CREATE POLICY "osis_foto_poster_insert" ON storage.objects FOR INSERT TO anon WITH CHECK (bucket_id='osis-foto' AND (storage.foldername(name))[1]='poster');
DROP POLICY IF EXISTS "osis_foto_poster_delete" ON storage.objects;
CREATE POLICY "osis_foto_poster_delete" ON storage.objects FOR DELETE TO anon USING (bucket_id='osis-foto' AND (storage.foldername(name))[1]='poster');
